...
function Requests() {
  ...
  async function fetchPendingRequests() {
    ...
    try {
      ...
      // Show only pending status requests
  ...
  // ─── Fetch owner data (for Send Request cascading dropdowns) ──────────────
      ...
      <div className="mb-8 flex items-end justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Manage incoming requests and assign issues to employees
          </p>
        </div>
        <button
          onClick={() => { fetchPendingRequests(); fetchOwnerData(); }}
